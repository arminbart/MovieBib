...
function get_cover_filename($id, $session = null)
{
	...
	if (!file_exists($filename) or !$session->valid())
...
function get_forward_page($from, $session = null)
{
	$params = explode(";", $from);
	$forward = $params[0] . ".php";
	if ($session != null)
		$forward .= $session->param();
	else
		$forward .= Session::session_param(null);
